...
export class NewControllerStoryboard extends Storyboard<ApiJson> {
  constructor(
    ...
    this.addFrame(new DefineControllerNameAndEndpointFrame(config, texts))
      .addFrame(
        new DefineControllerHandlersFrame(config, writeMethods, texts),
        (t) => {
      ...
      .addFrame(
        new CreateRoutesForHandlersFrame(config, writeMethods, texts),
        (t) => {
          const { name, endpoint } = t.getFrame(0).output;
      ...
      .addFrame(
        new CreateHandlerIoWithRouteFrame(config, writeMethods, texts),
        (t) => {
          const { name, endpoint } = t.getFrame(0).output;
          const { handlers, models, entities } = t.getFrame(1).output;
      ...
      .addFrame(
        new CreateControllerFrame(config, command, writeMethods, texts),
        (t) => {
          const { name, endpoint } = t.getFrame(0).output;
