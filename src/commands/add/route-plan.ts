...
export interface NamedRouteControllerPlan extends FeatureRouteControllerPlan {
...
export function createRouteEntry(plan: AddRoutePlan): RouteRegistryEntry {
  ...
  return {
    ...
    controller: plan.controller ? createNameVariants(plan.controller).kebabName : undefined,
...
export function createFeatureRouteControllerFile(plan: FeatureRouteControllerPlan): PlannedFile {
  ...
  return {
    ...
    content: createGroupedRouteControllerTs({ ...plan, controllerName: plan.resource.name }),
  };
}

export function createNamedRouteControllerFile(plan: NamedRouteControllerPlan): PlannedFile {
  const resourceNames = createNameVariants(plan.resource.name);
  const controllerNames = createNameVariants(plan.controllerName);

  return {
    path: path.posix.join(plan.featuresRoot, resourceNames.kebabName, "api", `${controllerNames.kebabName}.controller.ts`),
    type: "route",
    owner: resourceNames.kebabName,
    content: createGroupedRouteControllerTs(plan),
  };
}
...
function createGroupedRouteControllerTs(plan: NamedRouteControllerPlan): string {
  const resourceNames = createNameVariants(plan.resource.name);
  const controllerNames = createNameVariants(plan.controllerName);
...
@Controller('${plan.resource.path}', {
  apiDoc: {
    tags: ['${resourceNames.pascalName}'],
    description: '${controllerNames.pascalName} routes generated by SoapJS CLI',
  },
})
export class ${controllerNames.pascalName}Controller {${constructor}
${methods}
}
`;
}
...
function createFeatureRouteModel(plan: FeatureRouteControllerPlan, route: RouteRegistryEntry): FeatureRouteModel {
  ...
  const base: FeatureRouteModel = {
    decorator,
    routePath,
    resourceName: resourceNames.kebabName,
...
function createFeatureRouteMethodTs(model: FeatureRouteModel): string {
  ...
  async ${model.methodName}(): Promise<unknown> {
    return { resource: '${model.resourceName}', action: '${model.methodName}' };
  }`;
}
...
function resolveRoutePath(resourcePath: string, routeName: string, routePath?: string): string {
  ...
  throw new CliError(`Route path "${routePath}" must be under feature path "${resourcePath}".`);
}
